...
- optionally sets up the **Outlook calendar overlay** — it asks first, and if you say yes it signs you in to Microsoft 365 (Windows/WAM via WorkIQ, or a broker-free device-code fallback) and schedules a refresh.
...
**Data** is generated by `calendar/Update-Calendar.ps1`, which writes `calendar/calendar-events.js` (git-ignored — it contains your personal calendar). It supports two ways to reach your Outlook/M365 calendar and picks automatically:

| Backend | How it signs in | When it's used |
| --- | --- | --- |
| **WorkIQ** (`-Auth WorkIQ`) | [WorkIQ](https://www.npmjs.com/package/@microsoft/workiq) CLI via the Windows **WAM broker** (needs Node.js) | Primary — reuses your existing Windows M365 sign-in |
| **Graph** (`-Auth Graph`) | **Broker-free** Microsoft Graph — **device code** by default (browser with `-PreferBrowser`), then silent refresh. No Node, no WAM, no Azure CLI | Fallback for machines/tenants where WAM fails (e.g. `ApiContractViolation`) |

`-Auth Auto` (the default) tries **WorkIQ/WAM first, then broker-free Graph**. Run `-Login` once to set up the broker-free device-code sign-in; the refresh token is cached (DPAPI, per-user) in `calendar/graph-token.dat` so the scheduled refresh stays silent. For locked-down tenants you can point Graph at an admin-consented app with `-GraphClientId`/`-GraphTenant`.

> **Note:** some tenants enforce a Conditional Access **token protection** policy that *requires* the WAM broker — there, only the WorkIQ/WAM backend works and broker-free Graph is blocked by design.

```powershell
# One-off refresh (Auto: WAM first, then broker-free Graph)
.\calendar\Update-Calendar.ps1

# Set up broker-free device-code sign-in (no Node, no WAM)
.\calendar\Update-Calendar.ps1 -Login
.\calendar\Update-Calendar.ps1 -Login -PreferBrowser        # use the browser instead
.\calendar\Update-Calendar.ps1 -Auth Graph                  # force the Graph backend
...
`-Install` runs the tray at login via `Start-CalendarTray.vbs` (launched by `wscript.exe`), so PowerShell starts **fully hidden with no console window**. For a manual no-console start you can also double-click `calendar\Start-CalendarTray.vbs` instead of running the `.ps1` directly.
